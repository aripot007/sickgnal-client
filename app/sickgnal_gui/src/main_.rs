...
fn setup_chat_callbacks(
    ...
    // cancel_new_conversation — closes the dialog
    {
        ...
        ui.global::<Chat>().on_cancel_new_conversation(move || {
            // This will prove the callback bridged over from Slint to Rust
            println!("-> 1. Rust callback triggered!"); 

            let Some(ui) = ui_weak.upgrade() else { 
                println!("-> ERROR: ui is None!"); 
                return 
            };
            ...
            // This will prove the properties were successfully updated
            println!("-> 2. Properties updated to false in Rust!"); 
        });
    }
