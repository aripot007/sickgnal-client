...
use crate::{chat::message::{ChatMessage, ChatMessageKind, ControlMessage}, e2e::{client::{E2EClient, Error}, kdf::kdf, keys::{KeyStorageBackend, SymetricKey}, message::{E2EMessage, KeyExchangeData, encrypted_payload::{EncryptedPayload, PayloadMessage}}, message_stream::E2EMessageStream}};
    ...
    MsgStream: E2EMessageStream + Send,
{
    ...
    pub async fn next(&mut self) -> Result<Option<ChatMessage>, Error> {
        ...
        for (user_id, keys) in self.undecipherable_messages.iter() {
            let keys_strs: Vec<String> = keys.iter()
                .map(|(id, msgs)| format!("{} ({} messages)", id, msgs.len()))
                .collect();
            
            // TODO: Better logging
            println!("Unknown keys for user {} : {}", user_id, &keys_strs.join(","));
        }
    ...
    async fn process_messages(&mut self, messages: Vec<E2EMessage>) {
        ...
        while let Some(msg) = queue.pop_front() {
            ...
            match msg {
                E2EMessage::ConversationOpen { sender_id, data } => self.process_open_conversation(sender_id, data).await,
                E2EMessage::ConversationMessage { sender_id, msg_ciphertext } => self.process_conversation_message(sender_id, msg_ciphertext, &mut queue).await,
                _ => todo!()
            };
        }
    ...
    async fn process_open_conversation(&mut self, sender_id: Uuid, data: KeyExchangeData) {
        ...
        if !matches!(m.kind, ChatMessageKind::Ctrl(ControlMessage::OpenConv { .. })) {
    ...
    async fn process_conversation_message(&mut self, sender_id: Uuid, ciphertext: EncryptedPayload, queue: &mut VecDeque<E2EMessage>) {
        ...
        if let Some(key) = session_keys.keys.get(&ciphertext.key_id) {
            ...
            match ciphertext.decrypt(&aead) {
                ...
                // Handle key rotation
                Ok(PayloadMessage::E2EMessage(E2EMessage::KeyRotation { nonce, key_id, padding: _  })) => {
                    self.process_key_rotation(sender_id, ciphertext.key_id, &nonce, key_id, queue).await
                },
    ...
    async fn process_key_rotation(&mut self, sender_id: Uuid, previous_key_id: Uuid, nonce: &[u8], next_key_id: Uuid, queue: &mut VecDeque<E2EMessage>) {
        ...
        if let Some(key_queues) = self.undecipherable_messages.get_mut(&sender_id) {
            if let Some(key_queue) = key_queues.remove(&next_key_id) {
                ...
                for msg_ciphertext in key_queue {
                    let m = E2EMessage::ConversationMessage { sender_id, msg_ciphertext };
                    queue.push_front(m);
                }
            }
        }
