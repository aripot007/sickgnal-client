...
use crate::e2e::{message::E2EMessage, message_stream::{E2EMessageStream, MessageStreamError}};
    ...
    S: AsyncRead + AsyncWrite + Send + Unpin
{
    ...
    async fn send(&mut self, message: E2EMessage) -> Result<(), MessageStreamError> {
        ...
        self.byte_stream.write_vectored(&[IoSlice::new(&len.to_be_bytes()), IoSlice::new(&payload)])
            .await
            .map_err(Error::from)?;
    ...
    async fn receive(&mut self) -> Result<E2EMessage, MessageStreamError> {
        ...
        self.byte_stream.read_exact(&mut len)
            .await
            .map_err(Error::from)?;
        ...
        self.byte_stream.read_exact(&mut payload)
            .await
            .map_err(Error::from)?;

        let msg: E2EMessage = serde_json::from_slice(&payload)
            .map_err(Error::from)?;
